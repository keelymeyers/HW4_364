...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/keelymHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
def get_gifs_from_giphy(search_string):
    ...
    params={'api_key':api_key, 'limit':5, 'q': search_string}
    response = requests.get(baseurl,params=params)
    gifsInResponse = json.loads(response.text)['data']
    return gifsInResponse
...
def get_or_create_search_term(db_session, term, gifs = []):
    ...
    if searchTerm:
        ...
        giphy_list = get_gifs_from_giphy(term)
        for g in giphy_list:
...
def index():
    ...
    if request.method=='POST':
        if form.validate_on_submit():
            return redirect(url_for('search_results',search_term=get_or_create_search_term(db.session,form.search.data)))
...
def create_collection():
    ...
    if request.method=='POST':
        gif_picks = form.gif_picks.data
        gif_objects = [get_gif_by_id(int(id)) for id in gif_picks]
        get_or_create_collection(db.session,current_user=current_user,gif_list=gif_objects,name=form.name.data)
